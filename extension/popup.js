...
const PRESETS={
  "rune-surge":{effect:"wave",color1:"#57d9ff",color2:"#f4c95d",speed:1.08,intensity:1}
};
const D={
...
el.preset.onchange=async()=>{const style=PRESETS[el.preset.value];if(!style)return;setForm(style);const h=normalizeHandle(el.handle.value);if(!h)return note("Rune Surge loaded. Enter a handle to apply it.");settings.profiles[h]=formStyle();await persist();note("Rune Surge applied to @"+h)};
